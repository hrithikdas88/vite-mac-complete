...
function App() {
  ...
  const handlemodalclose = () => window.electron.ipcRenderer.send("IdlemodalHasbeemclosed")
  ...
  return (
    <>
      <div className={"main-content"}>
        ...
        <Stopwatch startDetection={startDetection} stopDetection={stopDetection} /> 
      

        {activitypersent && <p className="activity-percentage">Your activity percentage is {activitypersent}%</p>}
      </div>
      <Modal
        show={showModal}
      ...
      >
        ...
        <Modal.Body>
          {idletime && (
            <p>
              You have been idle for {Math.round(idletime) > 1 ? `${Math.round(idletime)} minutes` : `${Math.round(idletime)} minute`}
            </p>
          )}
        </Modal.Body>
        <Modal.Footer>
